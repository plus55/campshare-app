...
export default async function AdminPage() {
  ...
  const [pendingListings, pendingInsurance] = await Promise.all([
    database
      .prepare(
    ...
    database
      .prepare(
        `SELECT hp.userId, hp.firstName AS hostFirstName, hp.insuranceProvider, hp.updatedAt,
                u.email AS hostEmail
         FROM host_profile hp
         JOIN user u ON u.id = hp.userId
         WHERE hp.insuranceStatus = 'pending'
         ORDER BY hp.updatedAt ASC`
      )
      .all<InsuranceRow>(),
  ]);
  ...
  return (
    <main className="min-h-screen px-4 py-12">
      <div className="mx-auto max-w-[1080px]">
        ...
        <h2 className="mb-1 font-serif text-xl text-forest-deep">Insurance awaiting review</h2>
        <p className="mb-4 text-sm text-stone">
          {pendingInsurance.results.length} host{pendingInsurance.results.length === 1 ? "" : "s"} pending
        </p>
        <div className="mb-8 overflow-x-auto rounded-2xl border border-line bg-cream">
          <table className="w-full border-collapse text-sm">
            <thead>
              <tr>
                {["Host", "Provider", "Submitted", ""].map((h) => (
            ...
            <tbody>
              {pendingInsurance.results.map((r) => (
                <tr key={r.userId} className="hover:bg-sand">
                  <td className="px-4 py-2.5">
                    <div className="text-charcoal">{r.hostFirstName}</div>
                    <div className="text-xs text-stone">{r.hostEmail}</div>
                  </td>
                  <td className="px-4 py-2.5 text-xs text-charcoal-soft">{r.insuranceProvider ?? "—"}</td>
                  <td className="px-4 py-2.5 text-xs text-stone">{formatDate(r.updatedAt)}</td>
                  <td className="px-4 py-2.5">
                    <Link href={`/admin/insurance/${r.userId}`} className="text-clay hover:text-clay-deep text-sm">
                      Review →
                    </Link>
                  </td>
                </tr>
              ))}
              {pendingInsurance.results.length === 0 && (
                <tr>
                  <td colSpan={4} className="px-4 py-8 text-center text-stone">
                    No insurance awaiting review.
                  </td>
                </tr>
              )}
            </tbody>
            ...
            <tbody>
              {pendingListings.results.map((r) => (
                <tr key={r.id} className="hover:bg-sand">
                  ...
                  <td className="px-4 py-2.5 text-charcoal-soft">{fmtNzd(r.nightlyRate)}/night</td>
